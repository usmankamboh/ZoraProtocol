...
contract Market is IMarket {
    ...
    function isValidBid(uint256 tokenId, uint256 bidAmount)public view override returns (bool){
        ...
        return bidAmount != 0 && (bidAmount == splitShare(bidShares.creator, bidAmount)
            .add(splitShare(bidShares.prevOwner, bidAmount)).add(splitShare(bidShares.owner, bidAmount)));
    }
    ...
    function setBid(uint256 tokenId,Bid memory bid,address spender) public override onlyMediaCaller {
        ...
        if (_tokenAsks[tokenId].currency != address(0) && bid.currency == _tokenAsks[tokenId].currency &&
    ...
    function acceptBid(uint256 tokenId, Bid calldata expectedBid)external override onlyMediaCaller{
        ...
        require(bid.amount == expectedBid.amount && bid.currency == expectedBid.currency &&
            bid.sellOnShare.value == expectedBid.sellOnShare.value && bid.recipient == expectedBid.recipient,"Market: Unexpected bid found.");
